...
@Transactional
class CourtCaseDefendantStoreTest : IntegrationTestBase() {

  @Autowired
  lateinit var courtCaseDefendantStore: CourtCaseDefendantStore
  ...
  fun `persists a case-scoped defendant row and reads it back by defendant id`() {
    courtCaseDefendantStore.upsert(defendantOnCaseA, "20GD1234567", master, "John Doe", LocalDate.of(1980, 1, 31))
  ...
  fun `upserts in place on defendant id, storing identity as given including blank and null`() {
    courtCaseDefendantStore.upsert(defendantOnCaseA, "20GD1234567", master, "John Doe", LocalDate.of(1980, 1, 31))
    courtCaseDefendantStore.upsert(defendantOnCaseA, "20GD1234567", master, "", null)
  ...
  fun `keeps a row per case for the same person, preserving cross-case identity divergence`() {
    ...
    courtCaseDefendantStore.upsert(defendantOnCaseB, "20GD7654321", master, "Jon Doe", LocalDate.of(1980, 1, 31))
  ...
  fun `resolves a person on a specific case to their case-scoped defendant id`() {
    courtCaseDefendantStore.upsert(defendantOnCaseA, "20GD1234567", master, "John Doe", dob())
    courtCaseDefendantStore.upsert(defendantOnCaseB, "20GD7654321", master, "John Doe", dob())
