...
class PrisonerSearchEventListenerIntTest : IntegrationTestBase() {
  ...
  private fun ingestUnmatchedDocument() {
    HmctsCourtDefendantApiExtension.hmctsCourtDefendantApi.stubDefendants(
      CASE_REFERENCE,
      listOf(
        DefendantDetails(
          DEFENDANT_ID_NUMBER_WITH_MATCH_AFTER_CREATION,
          MASTER_DEFENDANT_ID_WITH_MATCH_AFTER_CREATION,
          "Some One",
          LocalDate.of(1990, 6, 1),
        ),
      ),
    )
    sendSubscriptionNotification(MASTER_DEFENDANT_ID_WITH_MATCH_AFTER_CREATION)
  }

  @Test
  fun `Test previously ingested warrant file matches once prisoner is created in NOMIS`() {
    ingestUnmatchedDocument()
    ...
    val file = courtDocumentRepository.findFirstByMasterDefendantIdOrderByIngestionAtDesc(MASTER_DEFENDANT_ID_WITH_MATCH_AFTER_CREATION)!!
  ...
  companion object {
    ...
    val MASTER_DEFENDANT_ID_WITH_MATCH_AFTER_CREATION = UUID.randomUUID()
  }
}
