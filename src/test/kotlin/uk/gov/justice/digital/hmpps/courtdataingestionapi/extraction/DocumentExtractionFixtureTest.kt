...
class DocumentExtractionFixtureTest {
  ...
  @Test fun `extracts defendant name`() = assertEquals("MORIARTY, James", result.headerFields["defendant name"])

  @Test fun `extracts date of birth`() = assertEquals("01/01/1860", result.headerFields["date of birth"])

  @Test fun `extracts age`() = assertEquals("35", result.headerFields["age"])

  @Test fun `extracts gender`() = assertEquals("Male", result.headerFields["gender"])

  @Test fun `extracts nationality`() = assertEquals("British", result.headerFields["nationality"])

  @Test fun `extracts aliases`() = assertEquals("THE NAPOLEON OF CRIME", result.headerFields["aliases"])

  @Test fun `extracts defendant address`() = assertEquals("23 Conduit Street, London", result.headerFields["defendant address"])

  @Test fun `extracts offences count`() = assertEquals("3", result.headerFields["offences"])

  // ── header: case fields ───────────────────────────────────────────────────

  @Test fun `extracts case reference`() = assertEquals("06MC0001825", result.headerFields["case reference"])

  @Test fun `extracts date of hearing`() = assertEquals("01/05/2026", result.headerFields["date of hearing"])

  @Test fun `extracts asn`() = assertEquals("2526MC0600000001M", result.headerFields["asn"])

  @Test fun `extracts defendant attendance`() = assertEquals("In person", result.headerFields["defendant attendance"])

  @Test fun `extracts prosecutor`() = assertEquals("Metropolitan Police", result.headerFields["prosecutor"])

  @Test fun `extracts jurisdiction`() = assertEquals("MAGISTRATES", result.headerFields["jurisdiction"])

  @Test fun `extracts hearing type`() = assertEquals("Crown Court Sending", result.headerFields["hearing type"])

  @Test fun `extracts post-hearing custody status`() = assertEquals("Remanded in custody", result.headerFields["post-hearing custody status"])

  @Test fun `extracts officer in case`() = assertEquals("DI G Lestrade", result.headerFields["officer in case"])

  // ── header: counsel with status disambiguation ────────────────────────────

  @Test fun `extracts prosecution counsel`() = assertEquals("S Holmes KC", result.headerFields["prosecution counsel"])

  @Test fun `disambiguates prosecution counsel status`() = assertEquals("Prosecution", result.headerFields["prosecution counsel status"])

  @Test fun `extracts defence counsel`() = assertEquals("I Adler KC", result.headerFields["defence counsel"])

  @Test fun `disambiguates defence counsel status`() = assertEquals("Defence", result.headerFields["defence counsel status"])

  // ── offence blocks ────────────────────────────────────────────────────────

  @Test fun `produces three offence blocks`() = assertEquals(3, result.offenceBlocks.size)
  ...
  @Test fun `document has two pages`() = assertEquals(2, result.pageCount)
  ...
  private fun offenceByCode(code: String) = result.offenceBlocks.firstOrNull { it["offence code"] == code }
}
